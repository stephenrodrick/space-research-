...
export default function SatelliteTracker() {
  ...
  const updateSatellitePositions = async () => {
    ...
    try {
      const limited = satellites.slice(0, 20)
      const updatedSatellites = await Promise.all(
        limited.map(async (satellite) => {
          try {
            const response = await fetch(
              `/api/satellites/position?id=${satellite.satid}&observer_lat=${userLat}&observer_lng=${userLng}&observer_alt=0&seconds=60`,
            )
            const data = await response.json()
            if (Array.isArray(data.positions) && data.positions.length) return data.positions[0]
            return satellite
          } catch (error) {
            return satellite
          }
