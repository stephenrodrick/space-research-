...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${N2YO_BASE_URL}/positions/${id}/${observerLat}/${observerLng}/${observerAlt}/${seconds}/?apiKey=${N2YO_API_KEY}`,
    )
    ...
    return NextResponse.json({ error: "Failed to fetch satellite position" }, { status: 500 })
  }
}
